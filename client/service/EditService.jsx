...
import {read, update} from './api-service.js'
...
read({
  ...
  const clickSubmit = () => {
    ...
    values.name && serviceData.append('name', values.name)
...
update({
  ...
  const handleChange = name => event => {
    ...
    const logoUrl = values.id
          ? `/api/services/logo/${values.id}?${new Date().getTime()}`
          : '/api/services/defaultphoto'
    if (values.redirect) {
    ...
    return (<div className={classes.root}>
      <Grid container spacing={8}>
        <Grid item xs={6} sm={6}>
          ...
          <Grid item xs={6} sm={6}>
 <MyProducts serviceId={params.serviceId} />
 </Grid>
